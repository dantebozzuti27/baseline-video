...
export async function POST(
  ...
  if (weeks && weeks.length > 0) {
    ...
    await supabase.from("program_template_weeks").insert(weekInserts);
  }
  ...
  if (days && days.length > 0) {
    ...
    await supabase.from("program_template_days").insert(dayInserts);
  }
  ...
  if (assignments && assignments.length > 0) {
    ...
    await supabase.from("program_template_day_assignments").insert(assignmentInserts);
  }
