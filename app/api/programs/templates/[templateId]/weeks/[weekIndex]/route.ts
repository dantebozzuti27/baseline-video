...
export async function PATCH(req: Request, { params }: { params: { templateId: string; weekIndex: string } }) {
  ...
  const { error } = await supabase.rpc("update_program_template_week", {
  ...
  if (error) {
    console.error("update_program_template_week failed", error);
